...
public class KernelSourceGenerator : IIncrementalGenerator
{
    ...
    {
        var metalCode = GenerateMetalShaderCode(method, compilation);
        
        // Also generate C# wrapper for Metal kernel integration
        var csharpWrapper = GenerateMetalCSharpWrapper(method, metalCode);
    ...
    /// <summary>
    /// Generates Metal compute shader code using the C# to Metal translator.
    /// </summary>
    private static string GenerateMetalShaderCode(KernelMethodInfo method, Compilation compilation)
    {
        // Use the new C# to Metal translator
        if (method.MethodDeclaration != null && compilation != null)
        {
            var semanticModel = compilation.GetSemanticModel(method.MethodDeclaration.SyntaxTree);
            var translator = new CSharpToMetalTranslator(semanticModel, method);
            
            try
            {
                return translator.GenerateCompleteKernel();
            }
            catch
            {
                // Fall back to basic generation if translation fails
            }
        }
        
        // Fallback to basic Metal shader generation
        var source = new StringBuilder();
        _ = source.AppendLine("// <auto-generated/>");
    ...
    private static string GenerateMetalCSharpWrapper(KernelMethodInfo method, string metalCode)
    {
        var source = new StringBuilder();
        _ = source.AppendLine("// <auto-generated/>");
        _ = source.AppendLine("// C# wrapper for Metal kernel integration");
        _ = source.AppendLine("using System;");
        _ = source.AppendLine("using DotCompute.Abstractions.Kernels;");
        _ = source.AppendLine("using DotCompute.Abstractions.Types;");
        _ = source.AppendLine();

        var namespaceParts = method.Namespace.Split('.');
        var generatedNamespace = string.Join(".", namespaceParts.Concat(["Generated", "Metal"]));

        _ = source.AppendLine($"namespace {generatedNamespace}");
        _ = source.AppendLine("{");
        _ = source.AppendLine($"    internal static class {method.Name}MetalKernel");
        _ = source.AppendLine("    {");
        _ = source.AppendLine("        public static string GetKernelSource()");
        _ = source.AppendLine("        {");
        _ = source.AppendLine("            return @\"" + metalCode.Replace("\"", "\\\"") + "\";");
        _ = source.AppendLine("        }");
        _ = source.AppendLine("        ");
        ...
        _ = source.AppendLine($"                EntryPoint = \"{method.Name}\",");
        _ = source.AppendLine("                Language = KernelLanguage.Metal");
        _ = source.AppendLine("            };");
        _ = source.AppendLine("        }");
        _ = source.AppendLine("    }");
        _ = source.AppendLine("}");
