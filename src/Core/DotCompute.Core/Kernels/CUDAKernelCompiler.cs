...
public sealed class CUDAKernelCompiler : DotCompute.Abstractions.IKernelCompiler
{
        ...
        try
        {
            ...
            unsafe
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < compilerOptions.Count; i++)
                    {
    ...
    private List<string> BuildNVRTCOptions(CompilationOptions options)
    {
        ...
        if (!string.IsNullOrEmpty(options.TargetArchitecture))
        {
            ...
            if (ComputeCapabilityVersions.TryGetValue(options.TargetArchitecture, out var archValue))
            {
    ...
    private static void ValidateBlockSize(int[]? blockSize, List<ValidationError> errors, List<ValidationWarning> warnings)
    {
        ...
        var totalSize = blockSize.Aggregate(1, (a, b) => a * b);
        if (totalSize > 1024)
        {
            errors.Add(new ValidationError
            {
    ...
    private static ResourceUsageEstimate EstimateResourceUsage(GeneratedKernel kernel)
    {
        ...
        var loopCount = CountSubstring(kernel.Source, "for") + CountSubstring(kernel.Source, "while");
    ...
    private static float CalculateOccupancyEstimate(int registers, int sharedMemory)
    {
        ...
        var registerLimitedOccupancy = registers > 0 ? Math.Min(1.0f, 65536.0f / (registers * 1024)) : 1.0f;
    ...
    private static string GenerateMockPTX(GeneratedKernel kernel, CompilationOptions options)
    {
        ...
        // Parameters
        for (var i = 0; i < kernel.Parameters.Length; i++)
        {
    ...
    {
        var baseRegisters = 12; // Base CUDA registers
        ...
        var complexityRegisters = sourceComplexity / 6; // CUDA is more efficient
