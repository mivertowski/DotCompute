...
namespace DotCompute.Core.Memory
{
        ...
        where TStatistics : class, new()
    {
        ...
        private readonly TStatistics _statistics;
        ...
        private void UpdateAllocationStatistics(long deltaBytes, bool isAllocation)
        {
            if (isAllocation)
            {
                _ = Interlocked.Add(ref _totalAllocatedBytes, deltaBytes);
                ...
                {
                    _ = Interlocked.Exchange(ref _peakUsageBytes, currentUsage);
                }
            }
            else
            {
                _ = Interlocked.Add(ref _totalAllocatedBytes, -deltaBytes);
                _ = Interlocked.Decrement(ref _allocationCount);
            }
