...
public sealed class P2PTransferScheduler : IAsyncDisposable
{
    ...
    private TransferStatistics _statistics;
        ...
        TransferStrategy strategy)
    {
        ...
        try
        {
            ...
            _deviceTransferCompletions.TryAdd(sourceDevice.Info.Id, new List<TaskCompletionSource<bool>>());
            _deviceTransferCompletions.TryAdd(targetDevice.Info.Id, new List<TaskCompletionSource<bool>>());
        }
        finally
        {
...
internal sealed class P2PTransferOperation<T> : P2PTransferOperation where T : unmanaged
{
    ...
    public override async Task ExecuteHostMediatedTransferAsync(CancellationToken cancellationToken)
    {
        ...
        await SourceBuffer.CopyToHostAsync<T>(fullSourceData, 0, cancellationToken);
        Array.Copy(fullSourceData, SourceOffset, hostData, 0, ElementCount);
        
        // Copy from host to target
        await TargetBuffer.CopyFromHostAsync<T>(hostData, TargetOffset, cancellationToken);
    }
