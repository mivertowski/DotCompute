...
namespace DotCompute.Abstractions.Interfaces.Kernels
{
    ...
    public sealed class GeneratedKernel
    {
        ...
        private static KernelMetadata CreateMetadataFromDict(Dictionary<string, object> dict)
        {
            ...
            {
                Name = dict.GetValueOrDefault("KernelName", "kernel")?.ToString() ?? "kernel",
                EntryPoint = dict.GetValueOrDefault("KernelName", "kernel")?.ToString() ?? "kernel",
                Parameters = Array.Empty<KernelParameterInfo>(),
                Attributes = new Dictionary<string, object>
                {
                    ["DeviceId"] = dict.GetValueOrDefault("DeviceId", "unknown"),
                    ["DeviceType"] = dict.GetValueOrDefault("DeviceType", "unknown"),
