    ...
    public static class SourceGeneratorHelpers
    {
        ...
        public static string GenerateParameterValidation(IEnumerable<(string name, string type, bool isBuffer)> parameters)
        {
            ...
            foreach (var (name, type, isBuffer) in parameters)
            {
                if (isBuffer)
                {
                    sb.AppendLine($"ArgumentNullException.ThrowIfNull({name}, nameof({name}));");
                }
                else if (type.Contains("Span") || type.Contains("Memory"))
                {
                    sb.AppendLine($"if ({name}.IsEmpty) throw new ArgumentException(\"Buffer cannot be empty\", nameof({name}));");
                }
            }
        ...
        public static string GenerateOptimizedLoop(string indexVar, string limitVar, string body, bool unroll = false, int unrollFactor = 4)
        {
            ...
            if (unroll)
            {
                ...
                sb.AppendLine($"var remainder = {limitVar} % {unrollFactor};");
                sb.AppendLine($"var limit = {limitVar} - remainder;");
                sb.AppendLine();
                ...
                sb.AppendLine("}");
                sb.AppendLine();
                sb.AppendLine("// Handle remainder");
                sb.AppendLine($"for (int {indexVar} = limit; {indexVar} < {limitVar}; {indexVar}++)");
                sb.AppendLine("{");
                sb.AppendLine($"    {body}");
                sb.AppendLine("}");
            }
            else
            {
                ...
                sb.AppendLine($"for (int {indexVar} = 0; {indexVar} < {limitVar}; {indexVar}++)");
                sb.AppendLine("{");
                sb.AppendLine($"    {body}");
                sb.AppendLine("}");
            }
