...
public sealed class ExpressionToKernelCompiler : IExpressionToKernelCompiler
{
    ...
    private List<Type> ExtractParameterTypes(Expression expression)
    {
    ...
    private string GenerateTemplateKey(ExpressionAnalysisResult analysis)
    {
    ...
    private static long EstimateTypeSize(Type type)
    {
        ...
        if (type.IsArray)
        {
            return EstimateTypeSize(type.GetElementType()!) * 1000; // Assume 1000 elements
        }
