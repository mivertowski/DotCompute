...
private unsafe float SumAvx2Float(float[] source)
{
    fixed (float* ptr = source)
    {
        ...
        float result = sum32.GetElement(0);
...
private unsafe float SumNeonFloat(float[] source)
{
    fixed (float* ptr = source)
    {
        ...
        // Horizontal sum
        float result = sum.GetElement(0) + sum.GetElement(1) + sum.GetElement(2) + sum.GetElement(3);
    ...
    where TKey : IComparable<TKey>
{
    if (left < right)
    {
        int pivotIndex = Partition(array, keySelector, left, right);
