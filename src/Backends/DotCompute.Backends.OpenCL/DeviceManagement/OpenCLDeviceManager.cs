...
internal sealed class OpenCLDeviceManager
{
    ...
    public OpenCLDeviceInfo? GetBestDevice()
    {
        ...
        if (bestDevice != null)
        {
            _logger.LogInformation("Selected best device: {DeviceName} ({DeviceType})", 
        ...
        try
        {
            ...
            foreach (var platformId in platformIds)
            {
                try
                {
                    ...
                    _logger.LogDebug("Added platform: {PlatformName} with {DeviceCount} devices", 
                        platform.Name, platform.Devices.Count);
                }
                catch (Exception ex)
                {
            ...
            _logger.LogInformation("Discovered {PlatformCount} platforms with {DeviceCount} total devices",
                platforms.Count, platforms.Sum(p => p.Devices.Count));
