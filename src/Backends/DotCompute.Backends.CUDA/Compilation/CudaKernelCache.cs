...
namespace DotCompute.Backends.CUDA.Compilation
{
    ...
    public sealed class CudaKernelCache : IDisposable
    {
            ...
            try
            {
                if (_memoryCache.TryGetValue(cacheKey, out var entry))
                {
                    ...
                    if (IsExpired(entry))
                    {
                        ...
                        try
                        {
                            _ = RemoveEntryInternal(cacheKey);
                            return false;
                        }
                        finally
                        {
        ...
        public async Task ClearAsync(CancellationToken cancellationToken = default)
        {
            ...
            {
                _ = _diskOperationsSemaphore.Release();
            }
        ...
        public static string GenerateCacheKey(string kernelName, byte[] sourceCode, CompilationOptions? options = null)
        {
            ...
            _ = keyBuilder.Append(kernelName).Append('_');
            _ = keyBuilder.Append(Convert.ToHexString(sourceHash));

            if (options != null)
            {
                _ = keyBuilder.Append('_').Append(options.OptimizationLevel);
                _ = keyBuilder.Append('_').Append(options.EnableDebugInfo ? "debug" : "release");
                ...
                if (options.AdditionalFlags?.Count > 0)
                {
                    ...
                    _ = keyBuilder.Append('_').Append(Convert.ToHexString(flagsHash)[..8]);
                }
            }
