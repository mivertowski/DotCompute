...
public static class CudaCompilerEnhancements
{
        ...
        int deviceId)
    {
        ...
        builder.AppendLine($"// Enhanced CUDA kernel: {source.Name}");
        ...
        builder.AppendLine($"// Optimization: {options?.OptimizationLevel ?? OptimizationLevel.Default}");
        builder.AppendLine();
        ...
        {
            builder.AppendLine("#include <cuda_fp16.h>");
            builder.AppendLine("#include <cuda_bf16.h>");
            builder.AppendLine("#include <cooperative_groups.h>");
            builder.AppendLine("#include <mma.h>"); // Matrix Multiply-Accumulate
        }

        if (major >= 8 && minor >= 9) // Ada generation
        {
            builder.AppendLine("#include <cuda_fp8.h>"); // FP8 support for Ada
        }

        if (major >= 9) // Hopper
        {
            builder.AppendLine("#include <cuda_pipeline.h>");
            builder.AppendLine("#include <cooperative_groups/memcpy_async.h>");
        }
        ...
        builder.AppendLine("// Architecture-specific performance macros");
        if (options?.OptimizationLevel == OptimizationLevel.Maximum)
        {
            ...
            if (major >= 8)
            {
                builder.AppendLine("#define CUDA_ENABLE_ASYNC_COPY");
            }
        }
        else
        {
        ...
        builder.AppendLine($"// Compute Capability {major}.{minor} Features");
        
        if (ComputeCapabilityExtensions.SupportsFeature(major, minor, ComputeFeature.TensorCores))
        {
            builder.AppendLine("#define TENSOR_CORES_AVAILABLE 1");
        }

        if (ComputeCapabilityExtensions.SupportsFeature(major, minor, ComputeFeature.CooperativeGroups))
        {
            builder.AppendLine("#define COOPERATIVE_GROUPS_AVAILABLE 1");
        }

        if (ComputeCapabilityExtensions.SupportsFeature(major, minor, ComputeFeature.AsyncCopy))
        {
            builder.AppendLine("#define ASYNC_COPY_AVAILABLE 1");
        }

        if (ComputeCapabilityExtensions.SupportsFeature(major, minor, ComputeFeature.FP8))
        {
            builder.AppendLine("#define FP8_AVAILABLE 1");
        }
        ...
        if (major >= 8) // Ampere and newer
        {
            ...
            builder.AppendLine($"#define MAX_SHARED_MEM_PER_BLOCK {ComputeCapabilityExtensions.GetMaxSharedMemoryPerBlock(major, minor)}");
            builder.AppendLine($"#define RECOMMENDED_BLOCK_SIZE {ComputeCapabilityExtensions.GetRecommendedBlockSize(major, minor)}");
        }

        if (major >= 8 && minor >= 9) // Ada generation specific
        {
            builder.AppendLine("#define ADA_OPTIMIZATIONS 1");
            builder.AppendLine("#define FP8_TENSOR_CORES_AVAILABLE 1");
        }

        if (major >= 9) // Hopper
        {
            builder.AppendLine("#define HOPPER_OPTIMIZATIONS 1");
            builder.AppendLine("#define CLUSTER_API_AVAILABLE 1");
        }
