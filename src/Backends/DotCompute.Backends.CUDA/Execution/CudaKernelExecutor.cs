...
public sealed class CudaKernelExecutor : IKernelExecutor, IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _activeExecutions.TryRemove(handle.Id, out _);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            await ExecuteAndWaitAsync(kernel, arguments, executionConfig, cancellationToken)
                .ConfigureAwait(false);
        }
    ...
    private CudaLaunchConfig GetOptimalLaunchConfig(CompiledKernel kernel, KernelExecutionConfig executionConfig)
    {
        ...
        switch (globalSize.Length)
        {
            case 1:
                return CudaLaunchConfig.Create1D(globalSize[0], localSize[0]);
            case 2:
                return CudaLaunchConfig.Create2D(globalSize[0], globalSize[1], localSize[0], localSize[1]);
            case 3:
                return CudaLaunchConfig.Create3D(globalSize[0], globalSize[1], globalSize[2], 
                    localSize[0], localSize[1], localSize[2]);
            default:
                throw new NotSupportedException($"Dimensions > 3 not supported: {globalSize.Length}");
        }
    }
    ...
    private List<string> GenerateOptimizationSuggestions(BottleneckAnalysis? bottleneck, KernelExecutionConfig config)
    {
