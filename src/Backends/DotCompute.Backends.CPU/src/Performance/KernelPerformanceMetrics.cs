...
public void RecordExecution(double executionTimeMs, bool useVectorization, int vectorWidth, PerformanceCounterSample startCounters, PerformanceCounterSample endCounters)
{
    ...
    if (useVectorization)
    {
        ...
        {
            case >= 512:
                _instructionSets.Add("AVX512F");
                break;
            case >= 256:
                _instructionSets.Add("AVX2");
                break;
            case >= 128:
                _instructionSets.Add("SSE");
                break;
        }
    }
...
private static string GenerateDetailedAnalysis(KernelPerformanceMetrics metrics)
{
    ...
    analysis.AppendLine($"Kernel '{metrics.KernelName}' Performance Analysis:");
    analysis.AppendLine($"- Execution Count: {metrics.ExecutionCount:N0}");
    analysis.AppendLine($"- Average Execution Time: {metrics.AverageExecutionTimeMs:F2} ms");
    analysis.AppendLine($"- Throughput: {metrics.ThroughputOpsPerSec:F2} ops/sec");
    analysis.AppendLine($"- Vectorization: {(metrics.VectorizationEnabled ? $"Enabled ({metrics.VectorWidth}-bit)" : "Disabled")}");
    analysis.AppendLine($"- Efficiency: {metrics.Efficiency:P1}");
    analysis.AppendLine($"- Cache Hit Ratio: {metrics.CacheHitRatio:P1}");
    analysis.AppendLine($"- Memory Bandwidth Utilization: {metrics.MemoryBandwidthUtilization:P1}");

    if (metrics.InstructionSets.Count > 0)
    {
        analysis.AppendLine($"- Instruction Sets: {string.Join(", ", metrics.InstructionSets)}");
    }
