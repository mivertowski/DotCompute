...
public interface IBuffer<T> : IMemoryBuffer where T : unmanaged
{
    ...
    IBuffer<T> Slice(int offset, int length);
    ...
    IBuffer<TNew> AsType<TNew>() where TNew : unmanaged;
    ...
    ValueTask CopyToAsync(IBuffer<T> destination, CancellationToken cancellationToken = default);
