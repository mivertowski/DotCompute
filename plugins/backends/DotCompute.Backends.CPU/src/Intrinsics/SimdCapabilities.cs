...
public static class SimdCapabilities
{
    ...
    private static HashSet<string> GetSupportedInstructionSets()
    {
        ...
        if (Bmi1.IsSupported) sets.Add("BMI1");
        if (Bmi2.IsSupported) sets.Add("BMI2");
        if (Popcnt.IsSupported) sets.Add("POPCNT");
        if (Lzcnt.IsSupported) sets.Add("LZCNT");

        // ARM instruction sets
        if (AdvSimd.IsSupported) sets.Add("NEON");
        if (AdvSimd.Arm64.IsSupported) sets.Add("NEON-ARM64");
        if (ArmBase.IsSupported) sets.Add("ARM-BASE");
        if (ArmBase.Arm64.IsSupported) sets.Add("ARM64-BASE");
        if (Crc32.IsSupported) sets.Add("CRC32");
        if (Crc32.Arm64.IsSupported) sets.Add("CRC32-ARM64");
        if (Dp.IsSupported) sets.Add("DP");
        if (Rdm.IsSupported) sets.Add("RDM");
        if (Sha1.IsSupported) sets.Add("SHA1");
        if (Sha256.IsSupported) sets.Add("SHA256");
...
public sealed class ArmSimdInfo
{
    ...
    public bool HasAes => Aes.IsSupported;
