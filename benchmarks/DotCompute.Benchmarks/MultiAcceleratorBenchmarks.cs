...
public class MultiAcceleratorBenchmarks
{
    ...
    public async Task MultiAcceleratorParallelExecution()
    {
        ...
        for (int i = 0; i < AcceleratorCount && i < _accelerators.Count; i++)
        {
            int acceleratorIndex = i;
    ...
    [Benchmark]
    public async Task WorkloadDistribution()
    {
        await ExecuteWorkloadType(WorkloadType);
    }
    ...
    private float[] CombinePartitionResults(float[][] partitionResults)
    {
    ...
    private async Task SimulateProcessingWork(IAccelerator accelerator, IMemoryBuffer buffer)
    {
