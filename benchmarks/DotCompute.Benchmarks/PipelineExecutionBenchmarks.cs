...
internal sealed class PipelineExecutionBenchmarks : IDisposable
{
    ...
    private readonly List<IKernelPipeline> _pipelines = [];
...
#pragma warning restore CA5394
    ...
    private async Task SetupPipelines()
    {
        ...
        for (var stages = 1; stages <= 10; stages++)
        {
    ...
    public async Task SingleStageExecution() => await ExecutePipelineWithStages(1);

    [Benchmark]
    public async Task MultiStageExecution() => await ExecutePipelineWithStages(PipelineStages);
    ...
    public async Task ParallelPipelineExecution()
    {
        ...
        for (var i = 0; i < parallelPipelines; i++)
        {
...
#pragma warning restore CA5394
    ...
    public async Task StreamingPipelineExecution()
    {
        ...
        for (var chunk = 0; chunk < streamChunks; chunk++)
        {
            var chunkIndex = chunk;
            tasks.Add(Task.Run(async () =>
            {
...
internal sealed class PipelineContext
{
    ...
    public IMemoryBuffer? TryGetBuffer(string key) => _buffers.TryGetValue(key, out var buffer) ? buffer : null;

    public void SetBuffer(string key, IMemoryBuffer buffer) => _buffers[key] = buffer;
}
