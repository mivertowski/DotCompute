...
public class PerformanceBenchmarks : IClassFixture<AcceleratorTestFixture>
{
    ...
    public async Task Benchmark_KernelLaunchOverhead()
    {
        ...
        var stopwatch = Stopwatch.StartNew();
        for (var i = 0; i < numSmallKernels; i++)
        {
    ...
    // Helper methods
    private static void VectorAddCPU(float[] a, float[] b, float[] c)
    {
    ...
    private static float ParallelReduction(float[] data)
    {
        ...
        Parallel.Invoke(
           ...
           () =>
           {
               for (var i = halfSize; i < data.Length; i++)
                   sum2 += data[i];
           }
        );
    ...
    private static double GetTheoreticalPeakGFLOPS()
    {
