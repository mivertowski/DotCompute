...
public sealed class ConcurrencyStressTests(ITestOutputHelper output) : IntegrationTestBase(output)
{
                        ...
                        switch (operationId % 4)
                        {
                            ...
                            case 3: // Free some buffers
                                if (localBuffers.Count > 5)
                                {
                                    ...
                                    await bufferToFree.DisposeAsync();
                                }
                                break;
                        }
    ...
    public async Task MemoryAllocation_UnderPressure_ShouldHandleGracefully()
    {
        ...
        var tasks = Enumerable.Range(0, maxConcurrentAllocations).Select(i =>
            Task.Run(async () =>
            {
                try
                {
                    var buffer = await memoryManager.AllocateAsync(allocationSize);
        ...
        var tasks = Enumerable.Range(0, kernelCount).Select(kernelId =>
            Task.Run(async () =>
            {
                try
                {
                    ...
                    // Copy input data
                    await inputBuffer.CopyFromHostAsync<byte>(
                        MemoryMarshal.Cast<float, byte>(inputData.AsSpan()).ToArray().AsMemory());

                    // Simulate computation(copy data first)
                    var tempBytes = new byte[dataSize * sizeof(float)];
                    await inputBuffer.CopyToHostAsync(tempBytes.AsMemory());
                    await outputBuffer.CopyFromHostAsync<byte>(tempBytes.AsMemory());
                        ...
                        switch (operation)
                        {
                            ...
                            case 1: // Memory transfer
                                if (componentBuffers.Count >= 2)
                                {
                                    ...
                                    if (src != dst && src.SizeInBytes == dst.SizeInBytes)
                                    {
                                        ...
                                        await src.CopyToHostAsync(tempData.AsMemory());
                                        await dst.CopyFromHostAsync<byte>(tempData.AsMemory());
                                    }
                                }
                                break;
                            ...
                            case 2: // Data operations
                                if (componentBuffers.Count > 0)
                                {
                                    ...
                                    await targetBuffer.CopyFromHostAsync<byte>(data.AsMemory());
                                }
                                break;
    ...
    public async Task SystemLimits_ExceedingLimits_ShouldRecoverGracefully()
    {
        ...
        for (var i = 0; i < maxAttempts; i++)
        {
            try
            {
                var buffer = await memoryManager.AllocateAsync(1024 * 1024); // 1MB
        ...
        try
        {
            var recoveryBuffer = await memoryManager.AllocateAsync(1024 * 1024);
