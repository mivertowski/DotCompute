...
public class EndToEndWorkflowTests : IntegrationTestBase
{
    ...
    public async Task CompleteComputePipeline_WithProfiling_ShouldCaptureMetrics()
    {
        ...
        result.Metrics.ComputeUtilization.Should().BeGreaterOrEqualTo(0);
        result.Metrics.MemoryBandwidthUtilization.Should().BeGreaterOrEqualTo(0);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var buffer = await CreateInputBuffer(memoryManager, input);
            buffers.Add(buffer);
        ...
        if (buffers.Count > 0)
        {
            var resultData = await ReadBufferAsync<float[]>(buffers[0]);
    ...
    {
        var builder = new KernelPipelineBuilder("multi_stage_test");
