...
public class MemoryTransferTests : IntegrationTestBase
{
    ...
    public async Task MemoryTransfer_HostToDevice_ShouldTransferCorrectly(int dataSize)
    {
        ...
        var testData = GenerateTestData(dataSize / sizeof(float));
    ...
    private static float[] GenerateTestData(int size, int seed = 42)
    {
        ...
        return Enumerable.Range(0, size)
                        .Select(_ => (float)random.NextDouble() * 1000.0f)
                        .ToArray();
    }
    ...
    private async Task<IBuffer> CreateBufferOnDevice(IAccelerator accelerator, float[] data)
    {
        return await CreateInputBuffer(accelerator.Memory, data);
    }

    private async Task<IBuffer> CreateInvalidBuffer(IMemoryManager memoryManager, long size)
    {
        if (size < 0)
