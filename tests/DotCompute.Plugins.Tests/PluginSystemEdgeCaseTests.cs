...
public sealed class PluginSystemEdgeCaseTests : IDisposable
{
    ...
    public async Task UnloadPluginsAsync_ConcurrentUnloading_ShouldBeThreadSafe()
    {
        ...
        var tasks = plugins.Select(plugin =>
            Task.Run(async () =>
            {
                try
                {
                    await _pluginSystem.UnloadPluginAsync(plugin.Name);
    ...
    public async Task UnloadPluginAsync_WithFailingDisposal_ShouldContinueGracefully()
    {
        ...
        mock.Setup(p => p.DisposeAsync())
            .ThrowsAsync(new InvalidOperationException("Disposal failed"));

        await _pluginSystem.LoadPluginAsync(mock.Object);

        // Act & Assert
        var unloadAct = async () => await _pluginSystem.UnloadPluginAsync("FailingDisposePlugin");
        await unloadAct.Should().ThrowAsync<PluginUnloadException>();

        // Plugin should still be removed from the system despite disposal failure
        var loadedPlugins = _pluginSystem.GetLoadedPlugins();
        loadedPlugins.Should().NotContain(p => p.Name == "FailingDisposePlugin");
    }
    ...
    {
        // Act & Assert
        var act = async () => await _pluginSystem.LoadPluginAsync(null!);
    ...
    [Fact]
    public async Task LoadPluginAsync_DuplicatePlugin_ShouldThrowPluginAlreadyLoadedException()
    {
        // Arrange
        var mock = new Mock<IBackendPlugin>();
        ...
        // Assert
        await act.Should().ThrowAsync<PluginAlreadyLoadedException>();
    }
    ...
    public async Task DisposeAsync_WithLoadedPlugins_ShouldUnloadAllPlugins()
    {
        ...
        // Act
        await _pluginSystem.DisposeAsync();

        // Assert - All plugins should have been disposed
        foreach (var plugin in plugins)
        {
            plugin.Verify(p => p.DisposeAsync(), Times.Once);
        }
