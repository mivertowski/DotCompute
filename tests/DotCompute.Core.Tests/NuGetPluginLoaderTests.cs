...
{
    private readonly Mock<ILogger<NuGetPluginLoader>> _mockLogger;
    private readonly NuGetPluginLoader _pluginLoader;
    private readonly string _tempDirectory;

    public NuGetPluginLoaderTests()
    {
        _mockLogger = new Mock<ILogger<NuGetPluginLoader>>();
        _tempDirectory = Path.Combine(Path.GetTempPath(), $"DotCompute_Tests_{Guid.NewGuid():N}");
        Directory.CreateDirectory(_tempDirectory);
        _pluginLoader = new NuGetPluginLoader(_mockLogger.Object, _tempDirectory);
    }
    ...
    {
        // Act & Assert
        Assert.Throws<ArgumentNullException>(() => new NuGetPluginLoader(null!, _tempDirectory));
    }
    ...
    {
        // Act & Assert
        Assert.Throws<ArgumentNullException>(() => new NuGetPluginLoader(_mockLogger.Object, null!));
    }
    ...
    public void Constructor_WithInvalidPluginDirectory_ShouldThrowDirectoryNotFoundException()
    {
        ...
        // Act & Assert
        Assert.Throws<DirectoryNotFoundException>(() => new NuGetPluginLoader(_mockLogger.Object, invalidPath));
    }
    ...
    public async Task LoadPluginAsync_WithSecurityRestrictions_ShouldEnforceSecurityPolicy()
    {
        ...
        var restrictedLoader = new NuGetPluginLoader(_mockLogger.Object, _tempDirectory, 
...
public class NuGetPluginLoader : IDisposable
{
    ...
    public NuGetPluginLoader(ILogger<NuGetPluginLoader> logger, string pluginDirectory, PluginSecurityPolicy? securityPolicy = null)
    {
