...
public class CUDAKernelCompilerTests : IDisposable
{
    ...
    public async Task CompileAsync_WithNonCudaKernel_ShouldThrowArgumentException()
    {
        ...
        var kernel = new GeneratedKernel
        {
            Id = Guid.NewGuid(),
            Name = "test_kernel",
            Language = KernelLanguage.OpenCL, // Wrong language
            SourceCode = "__global__ void test() {}",
            EntryPoint = "test",
    ...
    public async Task CompileAsync_WithOptimizationLevels_ShouldRespectOptimizations()
    {
        ...
        Assert.NotNull(result);
        Assert.True(result.IsCompiled);
        Assert.Equal(OptimizationLevel.Aggressive, options.OptimizationLevel);
    }
    ...
    public async Task CompileAsync_WithMacroDefinitions_ShouldIncludeMacros()
    {
        ...
        Assert.NotNull(result);
        Assert.True(result.IsCompiled);
        Assert.Contains("BLOCK_SIZE", options.Macros.Keys);
    ...
    public async Task CompileAsync_WithComplexKernel_ShouldHandleAdvancedFeatures()
    {
        ...
        Assert.NotNull(result);
        Assert.True(result.IsCompiled);
        Assert.Contains("shared_memory_size", result.Metadata);
        Assert.Contains("registers_per_thread", result.Metadata);
    }
    ...
    public async Task CompileAsync_WithInvalidCudaCode_ShouldHandleCompilationErrors()
    {
        ...
        var kernel = new GeneratedKernel
        {
            ...
            SourceCode = "__global__ void test() { invalid_syntax_here; }", // Invalid CUDA syntax
            EntryPoint = "test",
            Metadata = new KernelMetadata()
        };
        var options = CreateValidCompilationOptions();
    ...
    private GeneratedKernel CreateValidCudaKernel(string name = "test_kernel")
    {
...
__global__ void test_kernel(float* input, float* output, int size) {
    ...
    private GeneratedKernel CreateComplexCudaKernel()
    {
        return new GeneratedKernel
        {
            Id = Guid.NewGuid(),
...
__global__ void complex_kernel(float* A, float* B, float* C, int N) {
    ...
    private GeneratedKernel CreateKernelWithIncludes()
    {
        return new GeneratedKernel
        {
            Id = Guid.NewGuid(),
...
__global__ void kernel_with_includes(float* input, float* output, int size) {
    ...
    if (idx < size) {
        output[idx] = sqrtf(input[idx]);
            ...
            EntryPoint = "kernel_with_includes",
            Metadata = new KernelMetadata()
        };
    }

    private GeneratedKernel CreateKernelWithSharedMemory()
    {
        return new GeneratedKernel
        {
            Id = Guid.NewGuid(),
