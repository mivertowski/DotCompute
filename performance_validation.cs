...
public static class PerformanceValidator
{
    ...
    private static void ValidateSimdCapabilities()
    {
        ...
        if (Sse.IsSupported) Console.WriteLine("✅ SSE");
        if (Sse2.IsSupported) Console.WriteLine("✅ SSE2");
        if (Sse3.IsSupported) Console.WriteLine("✅ SSE3");
        if (Ssse3.IsSupported) Console.WriteLine("✅ SSSE3");
        if (Sse41.IsSupported) Console.WriteLine("✅ SSE4.1");
        if (Sse42.IsSupported) Console.WriteLine("✅ SSE4.2");
        if (Avx.IsSupported) Console.WriteLine("✅ AVX");
        if (Avx2.IsSupported) Console.WriteLine("✅ AVX2");
        if (Avx512F.IsSupported) Console.WriteLine("✅ AVX512F");
        if (Fma.IsSupported) Console.WriteLine("✅ FMA");
        if (AdvSimd.IsSupported) Console.WriteLine("✅ NEON (ARM64)");
